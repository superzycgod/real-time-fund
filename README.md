...
需先配置环境变量（与本地开发一致），否则构建出的镜像中 Supabase 等配置为空。可复制 `env.example` 为 `.env.local` 并填入实际值；若不用登录/反馈功能可留空。

1. 构建镜像（构建时会读取当前环境或同目录 `.env.local` 中的变量）
```bash
...
#### docker-compose（会读取同目录 `.env.local` 作为 build-arg 与运行环境）
```bash
